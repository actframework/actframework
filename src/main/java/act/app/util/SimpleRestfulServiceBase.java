...
SimpleRestfulServiceBase<
     ...
     */
    protected void onListingEntities(Query<MODEL_TYPE, ?> q) {}
    ...
    public Iterable<MODEL_TYPE> list(int _page, int _pageSize, String _orderBy) {
        ...
        Query<MODEL_TYPE, ?> q = filter(dao, ActionContext.current());
        onListingEntities(q);
        if (_pageSize > 0) {
    ...
    public static <MODEL_TYPE> Query<MODEL_TYPE, ?> filter(Dao<?, MODEL_TYPE, ?> dao, ParamValueProvider paramValueProvider) {
